...
Hierarchy of gestures (NewMemes):
  1. Muerte    -> Cabeza girada >= 15° (muerte.png)
  2. Sol       -> Ambas manos frente al pecho, puntas de los dedos formando arco circular (sol.jpeg)
...
LOG = logging.getLogger("gesture_meme")
...
FACE_STALE_MS = 1200
DEBUG_STATUS_INTERVAL_MS = 500

# Head turn angle (yaw, in degrees)
...
class GestureState:
    def __init__(self):
        self.last_face = None  # { mouthCenter, faceWidth, rightCheek, leftCheek, forehead, yawDeg, t }
    ...
    def update_face(self, face_result):
        ...
        self.last_debug["face_seen"] = saw_face
        self.last_debug["yaw"] = self.last_yaw_debug

    def decide(self, hand_result):
        now = time.time() * 1000
        face_is_fresh = self.last_face is not None and now - self.last_face["t"] < FACE_STALE_MS
        self.last_debug = {
            "face_seen": self.face_seen_this_frame,
            "face_fresh": face_is_fresh,
            "yaw": self.last_yaw_debug,
            "hands": len(hand_result.hand_landmarks or []),
        }

        # 1. Muerte — Cabeza girada ≥15° hacia la izquierda o derecha
        if face_is_fresh and abs(self.last_face["yawDeg"]) >= SIDE_EYE_YAW_DEG:
            self.last_debug["matched"] = "muerte"
            return "muerte"

        if not hand_result.hand_landmarks:
            self.last_debug["reason"] = "no_hands"
            self.last_debug["matched"] = "emperador"
            return "emperador"
        ...
        if len(hands) == 2:
            ...
            # 2. Sol — Ambas manos frente al pecho, puntas de los dedos enfrentadas y formando una abertura circular
            ...
            is_sol_pose = (
                ...
                sol_index_gap < 2.8 and (sol_thumb_gap < 3.2 or sol_middle_gap < 2.8) and
                palm_gap > index_dist * 1.05 and
            ...
            self.last_debug.update({
                "sol": is_sol_pose,
                "amantes": is_amantes_pose,
                "diablo": is_diablo_pose,
                "loco": is_loco_pose,
                "sol_index_gap": round(sol_index_gap, 2),
                "sol_thumb_gap": round(sol_thumb_gap, 2),
                "heart_index_gap": round(heart_index_gap, 2),
                "heart_thumb_gap": round(heart_thumb_gap, 2),
                "near_temples": near_temples,
                "both_index_up": both_index_up,
            ...
            if len(hands_above_head) == 1:
                ...
                if other_hand["palmCenter"][1] >= head_top_y:
                    self.last_debug["matched"] = "mago_2_hands"
                    return "mago"

            if is_amantes_pose:
                self.last_debug["matched"] = "amantes"
                return "amantes"

            if is_diablo_pose:
                self.last_debug["matched"] = "diablo"
                return "diablo"

            if is_loco_pose:
                self.last_debug["matched"] = "elLoco"
                return "elLoco"
        ...
        if len(hands) == 1:
            ...
            if h["palmCenter"][1] < head_top_y and h["wrist"][1] > h["palmCenter"][1]:
                self.last_debug["matched"] = "mago_1_hand"
                return "mago"

        # 7. Emperador — Cabeza frontal, manos abajo y brazos relajados a ambos lados del cuerpo (Default)
        self.last_debug["reason"] = "no_pose_threshold_matched"
        self.last_debug["matched"] = "emperador"
        return "emperador"


def load_memes():
    LOG.info("Cargando cartas desde: %s", MEMES.resolve())
    cache = {}
    for gesture, files in GESTURE_MEMES.items():
        ...
        for name in files:
            path = MEMES / name
            LOG.debug("Intentando cargar carta: %s | existe=%s", path, path.exists())
            img = cv2.imread(str(path))
            if img is None:
                LOG.error("No se pudo leer la carta: %s", path.resolve())
                raise FileNotFoundError(f"missing meme file: {path}")
            LOG.info("Carta OK: %-12s archivo=%s tamaño=%sx%s", gesture, name, img.shape[1], img.shape[0])
            imgs.append(img)
        cache[gesture] = imgs
    return cache
...
def draw_debug_hud(frame, state, gesture):
    lines = [
        f"Carta: {gesture}",
        f"Yaw cabeza: {state.last_yaw_debug:+.1f} deg  (Muerte thr +/-{SIDE_EYE_YAW_DEG:.1f})",
    ]
...
def main():
    LOG.info("=== INICIO GESTURE MEME ===")
    LOG.info("Python: %s", sys.version.replace("\n", " "))
    LOG.info("Script: %s", Path(__file__).resolve())
    LOG.info("ROOT: %s", ROOT.resolve())
    LOG.info("MODELS existe=%s ruta=%s", MODELS.exists(), MODELS.resolve())
    LOG.info("CARTAS existe=%s ruta=%s", MEMES.exists(), MEMES.resolve())
    for model_name in ("hand_landmarker.task", "face_landmarker.task"):
        model_path = MODELS / model_name
        LOG.info("Modelo %s existe=%s", model_name, model_path.exists())

    LOG.info("Creando detector de manos...")
    ...
    LOG.info("Detector de manos OK")
    LOG.info("Creando detector facial...")
    ...
    LOG.info("Detector facial OK")

    memes = load_memes()
    LOG.info("Todas las cartas cargadas: %s", list(memes.keys()))

    LOG.info("Abriendo cámara índice 0...")
    cap = cv2.VideoCapture(0)
    if not cap.isOpened():
        LOG.error("La cámara índice 0 no abrió. Revisa permisos de cámara para Terminal/Python.")
        raise RuntimeError("Could not open webcam (index 0)")
    LOG.info("Cámara OK | ancho=%s alto=%s fps=%s", cap.get(cv2.CAP_PROP_FRAME_WIDTH), cap.get(cv2.CAP_PROP_FRAME_HEIGHT), cap.get(cv2.CAP_PROP_FPS))
    ...
    try:
        while True:
            ok, frame = cap.read()
            if not ok:
                LOG.error("Falló cap.read() en frame=%s", frame_number)
                break
            frame_number += 1
            ...
            now = time.time() * 1000
            if now - last_status_log_at >= DEBUG_STATUS_INTERVAL_MS:
                LOG.debug(
                    "frame=%s detectado=%s actual=%s candidato=%s racha=%s datos=%s",
                    frame_number, gesture, current_gesture, candidate_gesture,
                    candidate_streak, state.last_debug,
                )
            ...
            if candidate_streak >= STABLE_FRAMES_REQUIRED and gesture != current_gesture:
                LOG.info(
                    "CAMBIO DE CARTA: %s -> %s | racha=%s | datos=%s",
                    current_gesture, gesture, candidate_streak, state.last_debug,
                )
            ...
            if gesture != "emperador":
                ...
                LOG.info("FALLBACK: %s -> emperador tras %d ms sin gesto", current_gesture, now - last_non_default_at)
                current_gesture = "emperador"
                current_meme = random.choice(memes["emperador"])
            ...
            if key == ord("q") or key == 27:
                LOG.info("Salida solicitada por teclado")
                break
    except Exception:
        LOG.exception("ERROR NO CONTROLADO dentro del loop principal")
        raise
    finally:
        LOG.info("Liberando cámara, ventanas y modelos...")
        cap.release()
...
if __name__ == "__main__":
    try:
        ...
        LOG.exception("El programa terminó por un error fatal")
        raise
